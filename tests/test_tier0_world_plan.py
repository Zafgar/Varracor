...
def test_plan_is_complete_structurally_and_acyclic():
    ...
    assert CURRENT_FOCUS == "low_fields"
...
def test_development_queue_starts_with_low_fields_and_keeps_partial_work_visible():
    queue = next_development_batch(20)
    assert queue
    assert queue[0]["area_id"] == "low_fields"
    assert queue[0]["domain"] in {"area", "quests", "dialogue", "graphics", "vfx", "persistence", "tests"}

...
class WeakAxe(Weapon):
    def __init__(self):
        super().__init__()
        self.name = "Iron Axe"
        self.rarity = "Common"
        self.cost = 50
        self.description = "A sturdy axe for splitting shields and skulls."
    ...
    def _load_image(self):
        path = "assets/gear/axes/weak_axe.png"
    ...
    def draw_card_icon(self, surface, x, y, size):
        ...
        if img:
            ...
            pygame.draw.line(surface, (150, 150, 160), (x + size*0.3, y + size*0.8), (x + size*0.7, y + size*0.2), 4)
    ...
    def release_charge(self, owner, manager, target_pos):
        # TAYSI LATAUS: WHIRLWIND - 360 asteen pyorahdys, osuu kaikkiin
        if self.charge_time >= self.max_charge:
            from systems import charge_specials
            charge_specials.whirlwind(owner, self, manager, target_pos)
            self.charge_time = 0
            return
        mult = 1.0 + (self.charge_time / self.max_charge) * 0.9 # Max 1.9x dmg
    ...
    def draw_equipped(self, surface, unit_rect, facing_right, attack_cooldown, total_cooldown=60, attack_vector=None):
        if self.charge_time > 15:
            ...
            pygame.draw.circle(surface, (255, 150, 100), (unit_rect.centerx, unit_rect.centery), 20 + glow_size, 2)
        ...
        if not self.image:
            ...
            pygame.draw.polygon(surface, (180, 180, 190), [(end_x, end_y), (end_x + 12, end_y - 12), (end_x + 12, end_y + 12)])
            return
        ...
        if attack_cooldown > 0 and attack_vector and prog < anim_duration:
            ...
            if anim_prog < 0.7:
                ...
                scale = 1.0 + (0.4 * math.sin(swing_pct * 3.14))
            else:

...
class WeakSpear(Weapon):
    def __init__(self):
        super().__init__()
        self.name = "Bent Spear"
        self.rarity = "Common"
        self.cost = 45
        self.description = "Long reach, but slightly crooked."
    ...
    def _load_image(self):
        path = "assets/gear/spears/weak_spear.png"
    ...
    def release_charge(self, owner, manager, target_pos):
        if self.charge_time > 15 and owner.attack_cooldown <= 0:
            # Stamina cost for dash
            if owner.current_stamina < 20: return
            owner.current_stamina -= 20
    ...
    def draw_equipped(self, surface, unit_rect, facing_right, attack_cooldown, total_cooldown=60, attack_vector=None):
        ...
        angle = -80 if facing_right else 80
        scale_y = 1.0

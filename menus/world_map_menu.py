...
class WorldMapMenu(BaseMenu):
    ...
    def on_enter(self):
        ...
        self.return_state = (
            getattr(self.manager, "world_map_return_state", "hub") or "hub"
        )
        self.selected_location = current_location_id(self.manager)
        self.feedback = ""
        self.feedback_timer = 0
    ...
    def update(self):
        ...
        self.btn_travel.update_hover(mouse)
    ...
    def _draw_regions(self, screen):
        for region in REGIONS.values():
            ...
            pygame.draw.polygon(screen, region["color"], points)
        ...
        for radius, color in (
            ...
            scaled = max(
                4,
                int(radius * self.map_rect.w / WORLD_MAP_SIZE[0]),
            )
        ...
        draw_text(
            ...
            (142, 178, 195),
            screen,
            self.map_rect.right - 155,
            self.map_rect.centery - 20,
        )
        draw_text(
            "SOUTHERN EXPANSE",
            font_small,
            (142, 178, 195),
            screen,
            self.map_rect.centerx - 75,
            self.map_rect.bottom - 30,
        )

    def _draw_routes(self, screen, state):
        current = state["current_location"]
        discovered_routes = set(state.get("discovered_routes", ()))
        for route in ROUTES:
            a, b = route["a"], route["b"]
            if route_key(a, b) not in discovered_routes:
                continue
    ...
    def _node_color(self, location_id, status):
        ...
        if status["visited"]:
            return (100, 160, 215)
        if status.get("route_discovered"):
            return (205, 155, 78)
        return (130, 130, 135)
    ...
    def _draw_nodes(self, screen, discovered):
        ...
        for location_id, location in LOCATIONS.items():
            ...
            if selected:
                pulse = 3 + int(
                    abs(math.sin(pygame.time.get_ticks() * 0.005)) * 4
                )
                pygame.draw.circle(
                    screen,
                    WHITE,
    ...
    def _draw_info(self, screen):
        ...
        draw_text(
            f"{region['name']}  |  {self._format_level(location['level_range'])}",
            font_small,
            (190, 190, 185),
            screen,
            x,
    ...
    def draw(self, screen):
        ...
        draw_text(
            ...
            f"Roads {summary['discovered_routes']}/{summary['total_routes']}",
            font_small,
            (205, 200, 185),
            screen,
            225,
        ...
        self._draw_routes(screen, state)
